        ...
        if len(matches['parents']) == 0:
            ...
            for pid in matches['parents']:
                ...
                try:
                    ...
                    logger.debug(f'failed indexing {place.id} {child_obj}')
                    sys.exit(sys.exc_info())
                ...
                try:
                    ...
                    logger.debug(f'failed updating {place.title} ({pid}) from child {place.id}')
                    sys.exit(sys.exc_info())
...
def init():
    ...
    try:
        ...
        logger.debug(f'failed deleting {dataset} from index {idx}: {ex}')
