...
//
// Both are therefore pinned here, to unicodedata 14.0.0 — the version the gateway and the index
// writer BOTH run. Measured directly, not inferred (place#284):
//
//   $ ps -u gazetteer -o pid,args | grep spawn_main
//     /home/gazetteer/miniconda/envs/whg/bin/python -c from multiprocessing...
//   $ /home/gazetteer/miniconda/envs/whg/bin/python -V   ->  3.11.13, unicodedata 14.0.0
//
// ⚠ THIS FILE PREVIOUSLY PINNED 13.0.0, AND ARGUED FOR IT AT LENGTH. That argument rested on a
// measurement of the HOST's system interpreter (`ssh pitt python3` → 3.9.25 / 13.0.0), not the
// service's: `scripts/gateway_ctl.sh` activates the `whg` conda env before `python -m gateway`,
// and a comment in that same file names the trap ("with cron's minimal PATH, `python` is system
// python3.9 ... only because THEY activate whg first"). There is no three-interpreter split and
// there never was; gateway and index writer agree. Retracted at source by the indexing session and
// re-measured here before this change.
//
// Impact of the correction was NIL, which is the only reason it was not urgent: a full-corpus scan
// found 0 of 73,479,069 toponyms containing any of the 515 codepoints that are alphabetic in
// 14.0.0 but not 13.0.0. The diff is one-directional — 515 gained, 0 lost — verified here rather
// than taken on report.
//
// The whitespace sets are identical across every version involved, so PY_WHITESPACE carries no
// such caveat.
//
// Regenerate ON THE GATEWAY'S OWN INTERPRETER (they no longer differ, but that is the one that
// receives our query_vector) — /home/gazetteer/miniconda/envs/whg/bin/python on `ssh pitt` — with:
//
//   python3 -c "rs=[];s=None
